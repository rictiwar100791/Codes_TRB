...
//This code corresponds to the single allocation (SA) version of the problem as described in Section 3.1. Also, the experimenst are run on standard data-sets of CAB 10, 15, 20 and 25, AP50.
...
void SOCP(int p, double al) //Refer to Section 4, Mixed Integer Second Order Conic Program (MISOCP)
{
	...
	Num2DMatrix Y_val(env, N);
	BoolVar2DMatrix Y(env, N);
	Num2DMatrix best_Y(env, N);
	...
	*/
	...
	//Constraint 1: for {i in 1..N}: sum {k in 1..p}(zz[i][k])=1;
	...
	//Constraint 4: for {i in 1..N}{j in 1..N}{k in 1..p}:sum {m in 1..p}(X[i][j][k][m])=zz[i][k]
	...
	for (int i = 0; i < N; i++)
	{
		for (int j = 0; j < N; j++)
		{
			for (int k = 0; k < N; k++)
			{
				...
				model.add(SumX == Y[i][k]);
				SumX.end();
			}
		}
	}

	//Constraint 5: for {i in 1..N}{j in 1..N}{m in 1..p}:sum {k in 1..p}(X[i][j][k][m])=zz[j][m];
	...
	*/
	...
	for (int i = 0; i < N; i++)
	{
		for (int k = 0; k < N; k++)
		{
			Y_val[i][k] = cplex.getValue(Y[i][k]);
		}
	}
	...
	cout << "The objective function to the SACOHLP is" << UB << endl;
	cout << "The market share captured by the entrant is " << MS << endl;

	cout << "==========================Optimal hubs==================================" << endl;
	for (int k = 0; k < N; k++)
	{
		if (Y_val[k][k] > eps)
		{
...
void CPA(int p, double al) //Refer to Appendix, Cutting Plane Algorithm (CPA) 
{
	...
	Obj.end(); // Clear out the memory allocated for the expression 
	// or is ||
	while (((1 - best_LB / best_UB) > .01) && (run_SB_tot / (double)CLOCKS_PER_SEC) < TIMLIM)
	{
		start_iter = clock();
		iter += 1;
		...
		//cout << "run_SB_tot " << run_SB_tot / (double)CLOCKS_PER_SEC << endl;
	}//end while
...

{
	...
	Num3DMatrix lb(env, N);//lagrange multiplier
	IloNumArray le(env, N);
	Num3DMatrix best_la(env, N);//best lagrange multiplier
	Num3DMatrix best_lb(env, N);//best lagrange multiplier
	IloNumArray best_le(env, N);
	...
	//XVAL *sorted2;//for storing the values of x[i][j] variables for sorting by qsort library function
	alpha = (XVAL *)calloc(N*N*N, sizeof(XVAL));
	beta = (XVAL *)calloc(N*N*N, sizeof(XVAL));
	epsilon = (XVAL *)calloc(N, sizeof(XVAL));
	sorted = (XVAL *)calloc(N, sizeof(XVAL));
	//sortie = (XVAL *)calloc(N*N, sizeof(XVAL));
	//sorted2 = (XVAL *)calloc(N*N, sizeof(XVAL));
	...
	le = IloNumArray(env, N, 0, IloInfinity, ILOFLOAT);
	best_le = IloNumArray(env, N, 0, IloInfinity, ILOFLOAT);
	for (int i = 0; i < N; i++)
	{
		la[i] = Num2DMatrix(env, N);
		lb[i] = Num2DMatrix(env, N);
		best_la[i] = Num2DMatrix(env, N);
		best_lb[i] = Num2DMatrix(env, N);
		for (int j = 0; j < N; j++)
		{
			la[i][j] = IloNumArray(env, N, 0, IloInfinity, ILOFLOAT);
			lb[i][j] = IloNumArray(env, N, 0, IloInfinity, ILOFLOAT);
			best_la[i][j] = IloNumArray(env, N, 0, IloInfinity, ILOFLOAT);
			best_lb[i][j] = IloNumArray(env, N, 0, IloInfinity, ILOFLOAT);
		}
	}
	...
	while ((1 - best_LB / best_UB) > .01)
	{
		...
		for (int k = 0; k < N; k++)
		{
			...
			for (int i = 0; i < N; i++)
			{
				...
				for (int j = 0; j < N; j++)
				{
					Sumsort += alpha[N*N*i + N*j + k].value + beta[N*N*j + N*i + k].value;
				}
				Sumsort -= le[i];
				Sumsort = Sumsort*Y_val[i][k];
	...
	https://www.tutorialspoint.com/c_standard_library/c_function_qsort.htm
		...
		for (int m = 0; m < p; m++)
		{
			Y_val[(sorted[m].index)][(sorted[m].index)] = 1;
		}

		for (int i = 0; i < N; i++)
		{
			le[i] = epsilon[i].value;
			best_le[i] = epsilon[i].value;
			for (int j = 0; j < N; j++)
			{
				for (int k = 0; k < N; k++)
				{
					la[i][j][k] = alpha[N*N*i + N*j + k].value;
					lb[i][j][k] = beta[N*N*i + N*j + k].value;
					best_la[i][j][k] = alpha[N*N*i + N*j + k].value;
					best_lb[i][j][k] = beta[N*N*i + N*j + k].value;
				}
			}
		}
	...
	IloNumArray Sumi(env, N);
	IloNumArray Sumi2(env, N);
	Num2DMatrix Sum(env, N);
	IloNumArray Sumk(env, N);

	//First way of generating a feasible solution
	/*
	for (int i = 0; i < N; i++)
	{
		Sum[i] = IloNumArray(env, N);
		Sumi[i] = IloInfinity;
	...
	*/
	...
	//Second way of generating a feasible solution
	...
	for (int i = 0; i < N; i++)
	{
		...
		for (int k = 0; k < N; k++)
		{
			...
			//Sumi2[i] = IloMin(Sum[i][k], Sumi2[i]);
			...
			for (int j = 0; j < N; j++)
			{
				...
				for (int k = 0; k < N; k++)
				{
					for (int l = 0; l < N; l++)
					{
						...
						Sumx += o[i][j][k][l]* Y_val[i][k] * Y_val[j][l];
					}
				}
		...
		for (int i = 0; i < N; i++)
		{
			for (int j = 0; j < N; j++)
			{
				...
				for (int k = 0; k < N; k++)
				{
					for (int l = 0; l < N; l++)
					{
						Obj_SUB2 -= la[i][j][k] * X[i][j][k][l] + lb[i][j][l] * X[i][j][k][l];
					}
				}
			}
		}
		...
		while (1 - (LB_SUB2 / UB_SUB2) > .01)
		{
			...
			for (int i = 0; i < N; i++)
			{
				for (int j = 0; j < N; j++)
				{
					R_new[i][j] = cplex_SUB2.getValue(R[i][j]);
			...
			for (int i = 0; i < N; i++)
			{
				for (int j = 0; j < N; j++)
				{
					for (int k = 0; k < N; k++)
					{
						for (int l = 0; l < N; l++)
						{
							Z_LB = Z_LB - (la[i][j][k] + lb[i][j][l])*X_val[i][j][k][l];
						}
					}
				}
			}
		...
		for (int i = 0; i < N; i++)
		{
			for (int j = 0; j < N; j++)
			{
				for (int k = 0; k < N; k++)
				{
					theta_SUB1 += (la[i][j][k] + lb[j][i][k]) * Y_val[i][k];
				}
			}
		}
		...
		for (int i = 0; i < N; i++)
		{
			for (int j = 0; j < N; j++)
			{
				for (int k = 0; k < N; k++)
				{
					...
					if (la[i][j][k] - t*(Y_val[i][k] - Sum1) > 0)
					{
						la[i][j][k] = la[i][j][k] - t*(Y_val[i][k] - Sum1);
				        //cout << "la[i][j][k] is" << la[i][j][k] << endl;
					}
					//else
					//{
						//la[i][j][k] = 0;
		...
		for (int i = 0; i < N; i++)
		{
			for (int j = 0; j < N; j++)
			{
				for (int l = 0; l < N; l++)
				{
					...
					if (lb[i][j][l] - t*(Y_val[j][l] - Sum1) > 0)
					{
						lb[i][j][l] = lb[i][j][l] - t*(Y_val[j][l] - Sum1);
					}
					//else
					//{
						//lb[i][j][l] = 0;
		...
		for (int i = 0; i < N; i++)
		{
			for (int j = 0; j < N; j++)
			{
				for (int k = 0; k < N; k++)
				{
					epsilon[i].value = le[i];
					alpha[N*N*i + N*j + k].value = la[i][j][k];
					beta[N*N*i + N*j + k].value = lb[i][j][k];
					if (count == 50)
					{
						epsilon[i].value = IloMax(best_le[i], 0);
						alpha[N*N*i + N*j + k].value = IloMax(0,best_la[i][j][k]);
						beta[N*N*i + N*j + k].value = IloMax(0,best_lb[i][j][k]);
						del = del / 2;
						cout << "------------------------------Delta Reduced-------------------------------------";
						count = 0;
					}
...
int main(int argc, char **argv)
{
	...
	cout << "=============================The CPA results are" << endl;
	CPA(p, alpha);

	cout << "=============================The LR-CPA results are" << endl;
	LR_CPA(p, alpha);
